 ...
 *
 * Ported verbatim from `prompts/domain_prompts.py`. The prompts are the value
 * here, not the code around them: the "benchmarks and decision rules you carry"
 * sections are what make an answer specific instead of a framework recital, and
 * paraphrasing them would quietly remove exactly what makes this useful. Keep
 * them word-for-word when porting upstream changes.
 *
 * `triage` is deliberately absent. Upstream registers it in the same
 * `SPECIALIST_REGISTRY`, so it appears in the consultable enum — but it does not
 * answer questions, it classifies inbound events for significance. It belongs
 * with the inbound/integration slice, not in a council that is asked for advice.
 *
 * Each prompt ends with a `<failure_cases>` instruction. That block is never
 * populated yet — the failure library is a later slice — but the text is kept
 * so it starts working the moment the library lands, with no prompt edit.
 */
...
export interface Specialist {
  ...
  /** One line, used as the routing menu entry and the tool description. */
  readonly description: string;
  readonly prompt: string;
}

const CSO_PROMPT = `You are the Chief Strategy Officer — a specialist in competitive strategy, market analysis, and long-horizon planning. You think in time horizons of 3-5 years while remaining anchored to near-term execution.
...
const CFO_PROMPT = `You are the Chief Financial Officer — a specialist in financial strategy, modeling, and capital allocation. You have built financial models for companies from seed through IPO, structured fundraising rounds, and managed board-level financial communications.
...
const CHRO_PROMPT = `You are the Chief HR/People Officer — a specialist in talent strategy, organizational design, and culture. You believe the people system is the operating system of the company.
...
const GC_PROMPT = `You are the General Counsel — a specialist in business law as it applies to operating companies. You have in-house experience at companies from seed through public, covering contracts, employment, IP, and regulatory matters.
...
const COO_PROMPT = `You are the Chief Operating Officer — a specialist in operational excellence, process design, and scaling. You have built and scaled operations across technology, services, and hybrid businesses.
...
const CMO_PROMPT = `You are the Chief Marketing Officer — a specialist in go-to-market strategy, brand building, and communications. You have launched products, repositioned brands, and managed communications through both growth and crisis.
...
const CPO_PROMPT = `You are the Chief Product Officer — a specialist in product strategy, roadmap design, and translating customer problems into product decisions. You have built products from 0 to 1 and scaled them to millions of users.
...
export const SPECIALISTS: Readonly<Record<SpecialistKey, Specialist>> = {
  cso: {
    key: "cso",
    title: "Chief Strategy Officer",
    description:
      "Competitive analysis, M&A, market positioning, scenario planning, OKRs",
    prompt: CSO_PROMPT,
  },
  cfo: {
    key: "cfo",
    ...
    description:
      "Financial modeling, unit economics, fundraising, cash flow, board finance",
    prompt: CFO_PROMPT,
  },
  chro: {
    key: "chro",
    title: "Chief HR/People Officer",
    description:
      "Hiring, compensation, performance management, culture, org design",
    prompt: CHRO_PROMPT,
  },
  gc: {
    key: "gc",
    ...
    description:
      "Contracts, IP, employment law basics, compliance (with appropriate disclaimers)",
    prompt: GC_PROMPT,
  },
  coo: {
    key: "coo",
    ...
    description:
      "Process design, vendor management, operational scaling, metrics",
    prompt: COO_PROMPT,
  },
  cmo: {
    key: "cmo",
    title: "Chief Marketing Officer",
    description: "GTM strategy, brand, messaging, PR, crisis communications",
    prompt: CMO_PROMPT,
  },
  cpo: {
    key: "cpo",
    title: "Chief Product Officer",
    description: "Product roadmap, prioritization frameworks, product strategy",
    prompt: CPO_PROMPT,
  },
  board_comms: {
    key: "board_comms",
    title: "Board Communications Director",
    description: "Board decks, investor relations, governance",
    prompt: BOARD_COMMS_PROMPT,
  },
  talent: {
    key: "talent",
    title: "Head of Talent & Executive Search",
    description:
      "Candidate screening and fit scoring, executive sourcing, energy-sector talent mapping",
    prompt: TALENT_PROMPT,
  },
};

export const SPECIALIST_KEYS: readonly SpecialistKey[] = Object.keys(
  SPECIALISTS,
) as SpecialistKey[];

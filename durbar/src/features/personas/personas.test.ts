...
describe("personas", () => {
  ...
  test("create custom persona", async () => {
    ...
    const { res, body } = await req(app, "/personas", {
      ...
      body: JSON.stringify({ display_name: "My Persona", body: "Custom body" }),
    });
  ...
  test("upsert persona", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body } = await req(app, "/personas/custom-test", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ display_name: "Custom Test", body: "Hello" }),
    });
  ...
  test("reset builtin", async () => {
    const db = openDb();
    const app = appWith(db);
    await req(app, "/personas/default", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ display_name: "Default", body: "Modified" }),
    });
    const { body } = await req(app, "/personas/default/reset", {
      method: "POST",
    });
    expect((body as { slug: string }).slug).toBe("default");
  });
  test("delete custom persona", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: created } = await req(app, "/personas", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ display_name: "Delete Me Persona", body: "Body" }),
    });

...
describe("fixtures", () => {
  ...
  test("generate requires description", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res } = await req(app, "/fixtures/generate", {
      method: "POST",
  ...
  test("create generated fixture", async () => {
    ...
    const { res, body } = await req(app, "/fixtures", {
      ...
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        bundle: { profile: { name: "Test Fixture Co" } },
      }),
    });
    expect(res.status).toBe(200);
  ...
  test("load unknown fixture 404", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res } = await req(app, "/fixtures/does-not-exist-xyz/load", {
      method: "POST",
    });
  ...
  test("invalid fixture name 400", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res } = await req(app, "/fixtures/BadName!/load", {
      method: "POST",
    });
    expect(res.status).toBe(400);
  });

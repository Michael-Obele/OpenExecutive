  ...
  try {
    const row = db
      .query<
        Record<string, unknown>,
        [string]
      >("SELECT 1 as x FROM generated_fixtures WHERE name = ? AND archived = 0")
      .get(name);
    return !!row;
  } catch {
    return false;
...
export function loadFixture(db: Db, name: string): Record<string, unknown> {
  ...
  return {
    loaded: name,
    status: "stubbed — full filesystem restore not yet wired",
  };
}
...
export function createGeneratedFixture(
  db: Db,
  data: {
    name: string;
    display_name: string;
...
export function deleteGeneratedFixture(db: Db, name: string): boolean {
  try {
    const result = db.run(
      "UPDATE generated_fixtures SET archived = 1, updated_at = ? WHERE name = ? AND archived = 0",

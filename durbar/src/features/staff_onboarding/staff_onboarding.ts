...
function rowToTemplate(row: Record<string, unknown>): OnboardingTemplate {
  ...
  try {
    brief = JSON.parse(
      (row["brief_sections_json"] as string) ?? "[]",
    ) as string[];
  } catch {
    brief = [];
  }
...
function rowToPlan(row: Record<string, unknown>): OnboardingPlan {
  ...
  try {
    reading = JSON.parse(
      (row["reading_list_json"] as string) ?? "[]",
    ) as string[];
  } catch {
    reading = [];
  }
  ...
  try {
    ramp = JSON.parse(
      (row["ramp_segments_json"] as string) ?? "[]",
    ) as string[];
  } catch {
    ramp = [];
  }
...
function rowToTask(row: Record<string, unknown>): OnboardingTask {
  return {
    ...
    completed_by_person_id:
      (row["completed_by_person_id"] as number | null) ?? null,
...
export function listTemplates(db: Db, activeOnly = true): OnboardingTemplate[] {
  ...
  const rows = db
    .query<
      ...
      []
    >(`SELECT * FROM onboarding_templates ${where} ORDER BY name`)
    .all();
...
export function getTemplate(db: Db, name: string): OnboardingTemplate | null {
  const row = db
    .query<
      Record<string, unknown>,
      [string]
    >("SELECT * FROM onboarding_templates WHERE name = ?")
    .get(name);
...
export function upsertTemplate(
  ...
  db.run(
    ...
    [
      ...
      JSON.stringify(data.task_specs),
      JSON.stringify(data.brief_sections),
      data.is_active ? 1 : 0,
      createdAt,
      now,
    ],
...
export function deleteTemplate(db: Db, name: string): boolean {
  const result = db.run("DELETE FROM onboarding_templates WHERE name = ?", [
    name,
  ]);
...
export function listPlans(
  db: Db,
  filters: { status?: OnboardingStatus; includeArchived?: boolean } = {},
): OnboardingPlan[] {
  ...
  const rows = params.length
    ? db
        .query<
          Record<string, unknown>,
          Array<string | number>
        >(`SELECT * FROM onboarding_plans ${where} ORDER BY id`)
        .all(...(params as Array<string | number>))
    : db
        .query<
          Record<string, unknown>,
          []
...
export function getPlan(db: Db, id: number): OnboardingPlan | null {
  const row = db
    .query<
      Record<string, unknown>,
      [number]
    >("SELECT * FROM onboarding_plans WHERE id = ?")
    .get(id);
...
export function createPlan(
  ...
  db.run(
    ...
    [
      ...
      data.start_date,
      data.person_id ?? null,
      data.manager_person_id ?? null,
      data.buddy_person_id ?? null,
      data.template_name ?? "",
      now,
      now,
    ],
...
export function updatePlan(
  ...
  const sets: string[] = [];
  const vals: unknown[] = [];
  if ("person_id" in patch) {
    sets.push("person_id = ?");
    vals.push(patch.person_id ?? null);
  }
  if ("manager_person_id" in patch) {
    sets.push("manager_person_id = ?");
    vals.push(patch.manager_person_id ?? null);
  }
  if ("buddy_person_id" in patch) {
    sets.push("buddy_person_id = ?");
    vals.push(patch.buddy_person_id ?? null);
  }
  if ("status" in patch && patch.status) {
    sets.push("status = ?");
    vals.push(patch.status);
  }
  if ("current_phase" in patch && patch.current_phase) {
    sets.push("current_phase = ?");
    vals.push(patch.current_phase);
  }
  if (sets.length === 0) return existing;
  vals.push(nowIso(), id);
  db.run(
    `UPDATE onboarding_plans SET ${sets.join(", ")}, updated_at = ? WHERE id = ?`,
...
export function advancePhase(db: Db, id: number): OnboardingPlan | null {
  ...
  if (next !== existing.current_phase) {
    db.run(
      "UPDATE onboarding_plans SET current_phase = ?, updated_at = ? WHERE id = ?",
      [next, nowIso(), id],
    );
  }
  return getPlan(db, id);
...
export function archivePlan(db: Db, id: number): boolean {
  const result = db.run(
    "UPDATE onboarding_plans SET archived = 1, updated_at = ? WHERE id = ? AND archived = 0",
    [nowIso(), id],
  );
  return result.changes > 0;
}
...
export function listTasks(db: Db, planId?: number): OnboardingTask[] {
  ...
  const rows = db
    .query<
      Record<string, unknown>,
      []
    >("SELECT * FROM onboarding_tasks ORDER BY plan_id, sort_order, id")
    .all();
  return rows.map(rowToTask);
}
...
export function getTask(db: Db, id: number): OnboardingTask | null {
  const row = db
    .query<
      Record<string, unknown>,
      [number]
    >("SELECT * FROM onboarding_tasks WHERE id = ?")
    .get(id);
  ...
  const maxOrder =
    db
      .query<
        { m: number | null },
        [number]
      >("SELECT MAX(sort_order) as m FROM onboarding_tasks WHERE plan_id = ?")
      .get(planId)?.m ?? -1;
  db.run(
...
export function setTaskStatus(
  ...
  db.run(
    "UPDATE onboarding_tasks SET status = ?, completed_at = ?, completed_by_person_id = ?, updated_at = ? WHERE id = ?",
    [status, completedAt, completedBy ?? null, nowIso(), id],
  );
...
export function activatePlan(db: Db, id: number): OnboardingPlan | null {
  ...
  if (!existing) return null;
  db.run(
    "UPDATE onboarding_plans SET status = 'active', updated_at = ? WHERE id = ?",
    [nowIso(), id],
  );
  return getPlan(db, id);
}

...
describe("staff onboarding — templates", () => {
  test("create and get template", async () => {
    ...
    const { res } = await req(app, "/onboarding-templates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "eng_onboarding", title: "Eng Onboarding" }),
    });
  ...
  test("list templates", async () => {
    const db = openDb();
    const app = appWith(db);
    await req(app, "/onboarding-templates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "tmpl_a", title: "A" }),
    });
    await req(app, "/onboarding-templates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "tmpl_b", title: "B" }),
    });
  ...
  test("delete template", async () => {
    ...
    await req(app, "/onboarding-templates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "to_delete", title: "Delete Me" }),
    });
    const { res } = await req(app, "/onboarding-templates/to_delete", {
      method: "DELETE",
    });
...
describe("staff onboarding — plans", () => {
  test("create and get plan", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res, body } = await req(app, "/onboarding-plans", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Alice", start_date: "2026-10-01" }),
    });
  ...
  test("advance and activate plan", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: created } = await req(app, "/onboarding-plans", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Bob", start_date: "2026-10-01" }),
    });
    const id = (created as { id: number }).id;
    const { body: advanced } = await req(
      app,
      `/onboarding-plans/${id}/advance`,
      { method: "POST" },
    );
    expect((advanced as { current_phase: string }).current_phase).toBe(
      "week_1",
    );
    const { body: activated } = await req(
      app,
      `/onboarding-plans/${id}/activate`,
      { method: "POST" },
    );
  ...
  test("archive plan", async () => {
    ...
    const { body: created } = await req(app, "/onboarding-plans", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Carol", start_date: "2026-10-01" }),
    });
    const id = (created as { id: number }).id;
    const { res } = await req(app, `/onboarding-plans/${id}/archive`, {
      method: "POST",
    });
    expect(res.status).toBe(204);
  });
...
describe("staff onboarding — tasks", () => {
  test("add and list tasks", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: plan } = await req(app, "/onboarding-plans", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Dave", start_date: "2026-10-01" }),
    });
    const pid = (plan as { id: number }).id;
    await req(app, `/onboarding-plans/${pid}/tasks`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ title: "Setup laptop" }),
    });
  ...
  test("task status update", async () => {
    ...
    const { body: plan } = await req(app, "/onboarding-plans", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Eve", start_date: "2026-10-01" }),
    });
    const pid = (plan as { id: number }).id;
    const { body: task } = await req(app, `/onboarding-plans/${pid}/tasks`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  test("activate and archive task", async () => {
    ...
    const { body: plan } = await req(app, "/onboarding-plans", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Frank", start_date: "2026-10-01" }),
    });
    const pid = (plan as { id: number }).id;
    const { body: task } = await req(app, `/onboarding-plans/${pid}/tasks`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ title: "Task X" }),
    });
    const tid = (task as { id: number }).id;
    const { body: activated } = await req(
      app,
      `/onboarding-tasks/${tid}/activate`,
      { method: "POST" },
    );
    expect((activated as { status: string }).status).toBe("in_progress");
    const { res } = await req(app, `/onboarding-tasks/${tid}/archive`, {
      method: "POST",
    });
    expect(res.status).toBe(204);
  });

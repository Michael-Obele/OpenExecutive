...
function rowToAction(row: Record<string, unknown>): ScheduledAction {
  return {
    ...
    assigned_to_person_id:
      (row["assigned_to_person_id"] as number | null) ?? null,
    awaiting_response_since:
      (row["awaiting_response_since"] as string | null) ?? null,
 ...
 * "cancelled".
 */
export function cancelScheduledAction(
  db: Db,
  id: number,
): "not_found" | "not_cancellable" | "cancelled" {
  const row = getScheduledAction(db, id);
  if (!row) return "not_found";
  if (row.status !== "pending") return "not_cancellable";
  db.run("UPDATE scheduled_actions SET status = 'cancelled' WHERE id = ?", [
    id,
  ]);
  return "cancelled";
}

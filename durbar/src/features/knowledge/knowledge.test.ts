...
describe("parseFrontmatter", () => {
  ...
  test("reads inline arrays as raw text", () => {
    expect(parseFrontmatter("failure_type: [a, b, c]")["failure_type"]).toBe(
      "[a, b, c]",
    );
  });
...
describe("splitSections", () => {
  ...
  test("handles a document with no headings", () => {
    expect(splitSections("just prose")).toEqual([
      { heading: "", body: "just prose" },
    ]);
  });
});
...
describe("index and search", () => {
  ...
  test("finds the board-deck skill from a question about preparing for a board meeting", () => {
    ...
    const hits = searchKnowledge(
      db,
      "I am preparing for a board meeting, what should be in my deck?",
    );
    expect(hits.some((hit) => hit.path.includes("board-prep-deck"))).toBe(true);
  });

  test("filters by domain", () => {
    const hits = searchKnowledge(db, "unit economics payback", {
      domain: "finance",
      limit: 5,
    });
    expect(hits.length).toBeGreaterThan(0);
  ...
  test("filters by kind", () => {
    const hits = searchKnowledge(db, "pricing strategy", {
      kind: "skill",
      limit: 5,
    });
  ...
  test("respects the limit", () => {
    expect(searchKnowledge(db, "strategy growth", { limit: 2 })).toHaveLength(
      2,
    );
  });
});

...
describe("talent — engagements", () => {
  test("create and get engagement", async () => {
    ...
    const { res, body } = await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "CTO" }),
    });
  ...
  test("list engagements", async () => {
    const db = openDb();
    const app = appWith(db);
    await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "Eng A" }),
    });
    await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "Eng B" }),
    });
  ...
  test("validation: role_title required", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res } = await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({}),
    });
  ...
  test("patch engagement", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: created } = await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "Old" }),
    });
    const id = (created as { id: number }).id;
    const { body: patched } = await req(app, `/engagements/${id}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "New" }),
    });
  ...
  test("archive engagement", async () => {
    ...
    const { body: created } = await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "ToArchive" }),
    });
    const id = (created as { id: number }).id;
    const { res } = await req(app, `/engagements/${id}/archive`, {
      method: "POST",
    });
    expect(res.status).toBe(204);
    const { body: list } = await req(app, "/engagements");
    expect((list as unknown[]).length).toBe(0);
    const { body: withArchived } = await req(
      app,
      "/engagements?include_archived=true",
    );
  ...
  test("matches stub returns empty", async () => {
    ...
    const { body: created } = await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "MatchMe" }),
    });
    const id = (created as { id: number }).id;
...
describe("talent — candidates", () => {
  test("create candidate requires engagement", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res } = await req(app, "/candidates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Alice", engagement_id: 9999 }),
    });
    expect(res.status).toBe(404);
  });
  test("create and list candidates", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: eng } = await req(app, "/engagements", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role_title: "Dev" }),
    });
    const eid = (eng as { id: number }).id;
    await req(app, "/candidates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Alice", engagement_id: eid }),
    });
    await req(app, "/candidates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Bob", engagement_id: eid }),
    });
  ...
  test("patch candidate", async () => {
    ...
    const eid = (eng as { id: number }).id;
    const { body: cand } = await req(app, "/candidates", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Alice", engagement_id: eid }),
    });
    const cid = (cand as { id: number }).id;
    const { body: patched } = await req(app, `/candidates/${cid}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Alicia" }),
    });
  ...
  test("stage transition", async () => {
    ...
    const cid = (cand as { id: number }).id;
    const { body: staged } = await req(app, `/candidates/${cid}/stage`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ stage: "screened" }),
    });
  ...
  test("archive candidate", async () => {
    ...
    const cid = (cand as { id: number }).id;
    const { res } = await req(app, `/candidates/${cid}/archive`, {
      method: "POST",
    });
    expect(res.status).toBe(204);
  });
...
describe("talent — offers", () => {
  test("create offer and enforce one-open invariant", async () => {
    ...
    const cid = (cand as { id: number }).id;
    const { res: r1 } = await req(app, "/offers", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ candidate_id: cid, comp_summary: "100k" }),
    });
    expect(r1.status).toBe(201);
    const { res: r2 } = await req(app, "/offers", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ candidate_id: cid, comp_summary: "120k" }),
    });
  ...
  test("extend and decide offer", async () => {
    ...
    const cid = (cand as { id: number }).id;
    const { body: offer } = await req(app, "/offers", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ candidate_id: cid, comp_summary: "100k" }),
    });
    const oid = (offer as { id: number }).id;
    const { body: extended } = await req(app, `/offers/${oid}/extend`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({}),
    });
    expect((extended as { offer: { status: string } }).offer.status).toBe(
      "extended",
    );
    const { body: decided } = await req(app, `/offers/${oid}/decision`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ decision: "accepted" }),
    });
    expect((decided as { offer: { status: string } }).offer.status).toBe(
      "accepted",
    );
  });

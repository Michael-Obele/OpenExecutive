...
function rowToOffer(row: Record<string, unknown>): Offer {
  ...
  try {
    ...
    if (raw)
      nudgeIds = (JSON.parse(raw) as unknown[])
        .map((x) => Number(x))
  ...
  return {
    ...
    approved_by_person_id:
      (row["approved_by_person_id"] as number | null) ?? null,
...
export function listEngagements(db: Db, includeArchived = false): Engagement[] {
  ...
  const rows = db
    .query<
      ...
      []
    >(`SELECT * FROM engagements ${where} ORDER BY id`)
    .all();
...
export function createEngagement(
  db: Db,
  data: Omit<Engagement, "id" | "archived" | "created_at" | "updated_at">,
): Engagement {
  const now = nowIso();
...
export function upsertEngagement(
  ...
  id: number | null,
  data: Partial<
    Omit<Engagement, "id" | "archived" | "created_at" | "updated_at">
  > & { role_title: string },
): Engagement {
  if (id !== null) {
...
export function listCandidates(
  ...
  if (filters.engagement_id !== undefined) {
  ...
  const rows = params.length
    ? db
        .query<
          Record<string, unknown>,
          Array<string | number>
        >(`SELECT * FROM candidates ${where} ORDER BY id`)
        .all(...params)
    : db
        .query<
          Record<string, unknown>,
...
export function setCandidateStage(
  db: Db,
  id: number,
  stage: CandidateStage,
): Candidate | null {
  const existing = getCandidate(db, id);
  if (!existing) return null;
  db.run("UPDATE candidates SET stage = ?, updated_at = ? WHERE id = ?", [
    stage,
    nowIso(),
    id,
  ]);
  ...
  const rows = params.length
    ? db
        .query<
          Record<string, unknown>,
          Array<string | number>
        >(`SELECT * FROM offers ${where} ORDER BY id`)
        .all(...params)
    : db
        .query<
          Record<string, unknown>,
...
export function getOpenOfferForCandidate(
  db: Db,
  candidateId: number,
): Offer | null {
  const row = db
...
export function createOffer(
  ...
  if (existing)
    throw new Error(
      `candidate ${data.candidate_id} already has an open offer (id ${existing.id})`,
    );
  const now = nowIso();
  try {
    db.run(
    ...
    if (msg.includes("UNIQUE") || msg.includes("unique"))
      throw new Error(
        `candidate ${data.candidate_id} already has an open offer`,
      );
    throw e;
  }
  const newId = Number(
...
export function updateOfferTerms(
  db: Db,
  id: number,
  patch: { comp_summary?: string | null; note?: string | null },
): Offer | null {
  const existing = getOffer(db, id);
  if (!existing) return null;
  if (!["draft", "pending_approval"].includes(existing.status))
    throw new Error(
      `Offer is ${existing.status}; terms are only editable while draft/pending_approval`,
    );
  const comp_summary =
    patch.comp_summary !== undefined && patch.comp_summary !== null
      ? patch.comp_summary
      : existing.comp_summary;
  const note =
    patch.note !== undefined && patch.note !== null
      ? patch.note
      : existing.note;
  db.run(
    "UPDATE offers SET comp_summary = ?, note = ?, updated_at = ? WHERE id = ?",
    [comp_summary, note, nowIso(), id],
  );
...
export function extendOffer(
  ...
  if (existing.status !== "draft" && existing.status !== "pending_approval")
    throw new Error(
      `Offer is ${existing.status}; only draft/pending_approval can be extended`,
    );
  let expiresAt: string | null = null;
  if (opts.expires_at) {
    const parsed = new Date(opts.expires_at);
    if (Number.isNaN(parsed.getTime()))
      throw new Error(`expires_at is not an ISO timestamp: ${opts.expires_at}`);
  ...
  const note =
    opts.note !== undefined && opts.note !== null ? opts.note : existing.note;
  db.run(
    "UPDATE offers SET status = 'extended', expires_at = ?, extended_at = ?, note = ?, updated_at = ? WHERE id = ?",
    [expiresAt, nowIso(), note, nowIso(), id],
  );
...
export function decideOffer(
  ...
  if (existing.status !== "extended")
    throw new Error(
      `Offer is ${existing.status}; only extended offers can be decided`,
    );
  if (!["accepted", "declined", "expired", "rescinded"].includes(decision))
    throw new Error(`Invalid decision status: ${decision}`);
  const finalNote = note !== undefined && note !== null ? note : existing.note;
  db.run(
    "UPDATE offers SET status = ?, decided_at = ?, note = ?, updated_at = ? WHERE id = ?",
    [decision, nowIso(), finalNote, nowIso(), id],
  );
  const updated = getOffer(db, id);
  if (!updated) throw new Error("Offer vanished");
  ...
  if (decision === "accepted") {
    db.run(
      "UPDATE candidates SET stage = 'placed', updated_at = ? WHERE id = ?",
      [nowIso(), existing.candidate_id],
    );
  }
  return updated;
}
...
export function validateEngagementCreate(
  body: unknown,
):
  | {
      ok: true;
  ...
  if (role_title.length > 200)
    return { ok: false, error: "role_title too long" };
...
export function validateCandidateCreate(
  body: unknown,
):
  | {
      ok: true;
  ...
  if (typeof engagement_id !== "number" || !Number.isInteger(engagement_id))
  ...
  return {
    ...
    data: {
      ...
      current_title:
        typeof b["current_title"] === "string" ? b["current_title"] : "",
      current_company:
        typeof b["current_company"] === "string" ? b["current_company"] : "",
      location: typeof b["location"] === "string" ? b["location"] : "",
      email:

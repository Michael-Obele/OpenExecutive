...
describe("agents", () => {
  ...
  test("override and clear", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: patched } = await req(app, "/agents/cfo/override", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role: "Custom CFO" }),
    });
    expect((patched as { role: string }).role).toBe("Custom CFO");
    const { res } = await req(app, "/agents/cfo/override", {
      method: "DELETE",
    });
  ...
  test("history", async () => {
    const db = openDb();
    const app = appWith(db);
    await req(app, "/agents/cfo/override", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role: "V1" }),
    });
    await req(app, "/agents/cfo/override", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ role: "V2" }),
    });
  ...
  test("test agent", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body } = await req(app, "/agents/cfo/test", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ query: "What is EBITDA?" }),
    });

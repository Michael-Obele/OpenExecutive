...
describe("evals", () => {
  test("list scenarios includes builtins", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body } = await req(app, "/evals/scenarios");
    // 42 builtin YAML scenarios are now loaded from evals/_scenarios/
    expect((body as { total: number }).total).toBe(42);
  });
  test("create scenario", async () => {
    const db = openDb();
    const app = appWith(db);
    const yaml = "id: test_001\nkind: chat\ndescription: test";
    const { res, body } = await req(app, "/evals/scenarios", {
      method: "POST",
  ...
  test("create run", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res, body } = await req(app, "/evals/runs", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  test("get run", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: created } = await req(app, "/evals/runs", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ kind: "chat" }),
    });

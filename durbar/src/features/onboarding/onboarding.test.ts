...
describe("onboarding", () => {
  ...
  test("message advances session", async () => {
    const db = openDb();
    const app = appWith(db);
    const { body: started } = await req(app, "/onboard/start", {
      method: "POST",
    });
    const sid = (started as { session_id: string }).session_id;
    const { body } = await req(app, "/onboard/message", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        session_id: sid,
        message: "We are a fintech startup",
      }),
    });
  ...
  test("404 for unknown session", async () => {
    const db = openDb();
    const app = appWith(db);
    const { res } = await req(app, "/onboard/message", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ session_id: "nope", message: "hi" }),
    });
  ...
  test("force draft and commit", async () => {
    ...
    const sid = (started as { session_id: string }).session_id;
    await req(app, "/onboard/message", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ session_id: sid, message: "We build rockets" }),
    });
    await req(app, "/onboard/message", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ session_id: sid, message: "Team of 10" }),
    });
    await req(app, "/onboard/message", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ session_id: sid, message: "Series A" }),
    });
    const { body: drafted } = await req(app, "/onboard/draft", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ session_id: sid }),
    });
    expect((drafted as { phase: string }).phase).toBe("draft");
    const { res } = await req(app, "/onboard/commit", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ session_id: sid }),
    });

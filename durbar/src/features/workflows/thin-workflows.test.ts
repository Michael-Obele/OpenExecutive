...
function fakeProvider(reply = "Model artifact."): Provider & { calls: ChatMessage[][] } {
...
const VALID_INPUTS: Record<string, Record<string, unknown>> = {
  annual_plan: {
    year_label: "FY 2026",
    prior_year_recap: "Closed at $4M ARR, launched mid-market motion, 14mo runway.",
    strategic_thesis: "Market shifting to ops-layer analytics, consolidate mid-market.",
  ...
  churn_deep_dive: { overview: "Churn 1.2% to 1.8%, concentrated in <50 seats", cohort_data: "<50: 3.1%, 50-200: 1.4%, >200: 0.6%" },
  comp_refresh: { driver: "Two finalists declined over comp band, eng bands stale 14mo", current_bands: "L3 $130-150K + 0.05%, L4 $160-185K + 0.08%", market_signal: "Bands 87% of median, equity 50% of Series-A median" },
  competitive_teardown: { competitor: "Mode Analytics", where_we_compete: "Mid-market BI, 100-500 person SaaS", our_perception: "They require central data team, we are self-serve" },
  crisis_comms: { incident_summary: "Security breach via compromised laptop, 14h access, 3% accounts", audiences: "50 enterprise accounts, 40 internal, press reached out" },
  engagement_value_report: {},
  exec_search_brief: { role_title: "VP Engineering", function: "Engineering", business_context: "Series A, 38 people, scaling mid-market", year_one_outcomes: "Ship platform tier, hire 10 eng, cut churn to 1%" },
  fundraising_prep: { round_label: "Series B $15M", traction_metrics: "ARR $4.1M +18% QoQ, NRR 112%, 38 headcount", narrative_thesis: "Analytics as ops-layer, mid-market consolidation", use_of_funds: "Platform tier, GTm expansion, 12 hires" },
  gtm_launch: { launch_name: "Workspaces GA", target_audience: "Mid-market RevOps at 100-500 person SaaS", value_proposition: "Team analytics in one place with IT controls", success_metrics: "40% adoption in 60d, $300K ARR, 3 case studies" },
  interview_coordination: { candidate_id: 1 },
  investor_update: { month_label: "May 2026", highlights: "Closed Acme $240K, shipped v2, hired VPE", lowlights: "Pipeline 2.1x, churn 1.8%", metrics: "ARR $4.1M, burn $660K, cash $9.2M, 14mo runway" },
  ma_evaluation: { target: "Acme Analytics, 20 people, $2M ARR, event pipeline", strategic_rationale: "Event pipeline fills our gap, 6mo faster than build" },
  mbr: { month_label: "May 2026", financial_actuals: "Revenue $340K vs $360K plan, burn $660K, cash $9.2M", kpi_movements: "NRR 112%, logo churn 1.8%, NPS 47", function_updates: "Eng: v2 shipped, GTM: 11 logos vs 14 plan, CS: NPS 47" },
  new_hire_onboarding: { candidate_id: 1 },
  offer_approval: { candidate_id: 1 },
  org_design: { current_structure: "CTO -> 3 eng leads, flat, 12 eng", proposed_structure: "Add VPE, 2 teams of 6, eng managers", motivation: "Span of control breaking, CTO bottleneck" },
  performance_review: { review_period: "H1 2026", manager_role: "VP Engineering", direct_reports: "Alice: shipped v2, strong. Bob: missed deadlines, needs coaching." },
  pricing_review: { proposed_change: "Add usage-based tier at $0.10 per 1K events", current_packaging: "Seat-based: $50/seat, 3 tiers", motivation: "Heavy-usage accounts underpay, SMB churn on price" },
  product_strategy: { horizon: "H2 2026", jobs_to_be_done: "RevOps: share reports, Data: self-serve query", current_bets: "Workspaces GA, AI query, platform tier" },
  quarterly_plan: { quarter_label: "Q3 2026", prior_quarter_recap: "Shipped v2, mid-market 2x pipeline, SOC2 done", top_strategic_priorities: "Land 5 mid-market, ship onboarding, cut churn", biggest_bets: "Mid-market motion, self-serve onboarding" },
  reference_check: { candidate_id: 1 },
  risk_register: { review_period: "FY 2026", business_context: "Series A, 38 people, $4M ARR, 14mo runway", known_concerns: "Key-person risk on CTO, churn in SMB, burn rate" },
  role_onboarding: {},
};
...
describe("runThinWorkflow — validation edge", () => {
  ...
  test("board_prep: missing required field", async () => {
    const db = openDb();
    const provider = fakeProvider("should not be called");
    const result = await runThinWorkflow("board_prep", { quarter_label: "Q2" }, { db, provider });
    expect(result.narrative).toContain("Missing required field");
    expect(provider.calls).toHaveLength(0);
  });
  ...
  test("candidate_screen: missing candidate_id", async () => {
    const db = openDb();
    const provider = fakeProvider("should not be called");
    const result = await runThinWorkflow("candidate_screen", { engagement_id: 1 }, { db, provider });
    expect(result.narrative).toContain("Missing required field");
    expect(provider.calls).toHaveLength(0);
  });
  ...
  test("engagement_value_report: no required fields, empty inputs still calls provider", async () => {
    ...
    const result = await runThinWorkflow("engagement_value_report", {}, { db, provider });
    expect(provider.calls).toHaveLength(1);
  ...
  test("role_onboarding: no required fields, empty inputs still calls provider", async () => {
    ...
    const result = await runThinWorkflow("role_onboarding", {}, { db, provider });
    expect(provider.calls).toHaveLength(1);
...
describe("runThinWorkflow — provider failure", () => {
  test("gracefully renders fallback artifact and still persists", async () => {
    ...
    const result = await runThinWorkflow("annual_plan", inputs, { db, provider });
    expect(result.narrative).toContain("Provider failed");
    expect(result.narrative).toContain("429");
    const row = db
      .query<{ status: string; artifact: string }, [string]>(
...
describe("runThinWorkflow — context assembly", () => {
  test("includes company context when profile exists", async () => {
    const db = openDb();
    db.run("INSERT INTO company_profile (id, data, updated_at) VALUES (1, ?, ?)", [
      JSON.stringify({ name: "Acme Corp", industry: "SaaS", stage: "Series A", mission: "Build great software" }),
      new Date().toISOString(),
    ]);
    const provider = fakeProvider("ok");
    await runThinWorkflow("annual_plan", VALID_INPUTS["annual_plan"] ?? {}, { db, provider });
  ...
  test("includes department context when departments exist", async () => {
    ...
    const provider = fakeProvider("ok");
    await runThinWorkflow("board_prep", VALID_INPUTS["board_prep"] ?? {}, { db, provider });
    const userContent = provider.calls[0]?.[1]?.content ?? "";
  ...
  test("uses provided runId", async () => {
    ...
    const result = await runThinWorkflow("annual_plan", VALID_INPUTS["annual_plan"] ?? {}, { db, provider, runId });
    expect(result.runId).toBe(runId);
    const row = db.query<{ run_id: string }, [string]>("SELECT run_id FROM workflow_runs WHERE run_id = ?").get(runId);
  ...
  test("updates existing run when runId already exists", async () => {
    ...
    db.run(
      ...
      [runId, "annual_plan", "Annual Plan", JSON.stringify({}), new Date().toISOString(), new Date().toISOString()],
    );
    const result = await runThinWorkflow("annual_plan", VALID_INPUTS["annual_plan"] ?? {}, { db, provider, runId });
    expect(result.runId).toBe(runId);

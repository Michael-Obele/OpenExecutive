...
function seedPerson(db: ReturnType<typeof openDb>, overrides: Record<string, unknown> = {}) {
  ...
  return Number((db.query("SELECT last_insert_rowid() as id").get() as Record<string, unknown>)["id"] as number);
}
...
describe("nudge engine", () => {
  ...
  test("stalled workflow candidate is nudged", () => {
    ...
    const now = new Date();
    const awaitingUntil = new Date(now.getTime() + 2 * 3600 * 1000).toISOString(); // 2h from now
    ...
    db.run(
      ...
      ["run-1", "offer_approval", "Offer for Bob", "awaiting_human", "{}", now.toISOString(), updatedAt, personId, awaitingUntil],
    );
    const n = runNudgeScan(db, now);
    expect(n).toBe(1);
    const nudges = db
      .query<Record<string, unknown>, []>("SELECT scope_key, kind FROM scheduled_actions WHERE kind = 'proactive_nudge'")
      .all();
  ...
  test("stalled workflow not nudged if deadline already passed", () => {
    ...
    db.run(
      "INSERT INTO workflow_runs (run_id, workflow_name, title, status, inputs, created_at, updated_at, awaiting_person_id, awaiting_until) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      ["run-1", "offer_approval", "Offer", "awaiting_human", "{}", now.toISOString(), now.toISOString(), personId, awaitingUntil],
    );
    const n = runNudgeScan(db, now);
    expect(n).toBe(0);
  });
  ...
  test("stalled workflow not nudged if recently updated (minQuiet)", () => {
    ...
    const personId = seedPerson(db);
    const now = new Date();
    const awaitingUntil = new Date(now.getTime() + 2 * 3600 * 1000).toISOString();
  ...
  test("stale commitment candidate is nudged", () => {
    ...
    const personId = seedPerson(db);
    const now = new Date();
    const awaitingSince = new Date(now.getTime() - 5 * 24 * 3600 * 1000).toISOString(); // 5 days ago
    db.run(
      ...
      [now.toISOString(), now.toISOString(), "email", "alice@example.com", "Please review", awaitingSince, personId],
    );
    const n = runNudgeScan(db, now);
    expect(n).toBe(1);
    const nudges = db
      .query<Record<string, unknown>, []>("SELECT scope_key FROM scheduled_actions WHERE kind = 'proactive_nudge'")
      .all();
    expect((nudges[0]!["scope_key"] as string).startsWith(SCOPE_PREFIX_COMMITMENT)).toBe(true);
  });

  test("idle initiative candidate is nudged", () => {
    const db = openDb();
    const personId = seedPerson(db);
    db.run("INSERT INTO departments (slug, title, head_person_id, updated_at) VALUES (?, ?, ?, ?)", [
      "eng",
      "Engineering",
      personId,
    ...
    const now = new Date();
    const updatedAt = new Date(now.getTime() - 10 * 24 * 3600 * 1000).toISOString(); // 10 days ago
    db.run("INSERT INTO initiatives (title, status, created_at, updated_at, department) VALUES (?, ?, ?, ?, ?)", [
      "Launch v2",
      "active",
      updatedAt,
      updatedAt,
    ...
    const nudges = db
      .query<Record<string, unknown>, []>("SELECT scope_key FROM scheduled_actions WHERE kind = 'proactive_nudge'")
      .all();
    expect((nudges[0]!["scope_key"] as string).startsWith(SCOPE_PREFIX_INITIATIVE)).toBe(true);
  });
  ...
  test("dedup: second scan does not re-nudge same scope within cooldown", () => {
    ...
    db.run(
      "INSERT INTO workflow_runs (run_id, workflow_name, title, status, inputs, created_at, updated_at, awaiting_person_id, awaiting_until) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      ["run-1", "offer_approval", "Offer", "awaiting_human", "{}", now.toISOString(), updatedAt, personId, awaitingUntil],
    );
  ...
  test("per-scope cap stops re-nudging after max", () => {
    ...
    for (let i = 0; i < 3; i++) {
      db.run(
        ...
        [new Date(now.getTime() - (i + 10) * 24 * 3600 * 1000).toISOString(), now.toISOString(), "email", "a@b.com", "nudge", `${SCOPE_PREFIX_STALLED}:run-1`],
      );
    }
  ...
  test("bootstrapNudgeScan inserts heartbeat", () => {
    ...
    const rows = db
      .query<Record<string, unknown>, []>("SELECT kind FROM scheduled_actions WHERE kind = 'nudge_scan'")
      .all();
  ...
  test("caps: maxPerScan limits emitted nudges", () => {
    ...
    const updatedAt = new Date(now.getTime() - 5 * 3600 * 1000).toISOString();
    for (let i = 0; i < 5; i++) {
      const pid = seedPerson(db, { full_name: `Person ${i}`, email: `p${i}@example.com` });
      db.run(
        "INSERT INTO workflow_runs (run_id, workflow_name, title, status, inputs, created_at, updated_at, awaiting_person_id, awaiting_until) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [`run-${i}`, "offer_approval", `Offer ${i}`, "awaiting_human", "{}", now.toISOString(), updatedAt, pid, awaitingUntil],
      );
    }

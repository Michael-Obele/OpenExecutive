...
const FALLBACK_SKILLS: Array<{
...
}> = [
  {
    ...
    description: "Evidence-first coding workflow",
    when_to_use: "Any task that writes or modifies code",
  ...
  },
  {
    ...
    when_to_use: "Building features test-first",
    body: "# TDD\nRed-green-refactor.",
  },
];

function listBuiltinSkills(): SkillDetail[] {
  return FALLBACK_SKILLS.map((s) => ({
    ...s,
    source: "builtin",
    filename: `${s.name}.md`,
  }));
...
export function searchSkills(
  ...
  const scored = skills
    .map((s) => {
      const hay =
        `${s.name} ${s.description} ${s.when_to_use} ${s.body}`.toLowerCase();
...
export function validateSkillName(name: string): string | null {
  if (!name || !/^[a-z0-9_-]+$/.test(name))
    return "Skill name must be kebab-case (a-z, 0-9, -, _)";
  return null;
}

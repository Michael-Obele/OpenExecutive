...
export const listArchivedArtifacts = query(async (): Promise<ArtifactSummary[]> => {
	const data = await durbarJson<{ artifacts: ArtifactSummary[] }>('/artifacts', {
		query: { archived: 'true' }
	});
	return data.artifacts;
...
export const getArtifact = query(v.string(), async (id): Promise<ArtifactDetail> => {
	return durbarJson<ArtifactDetail>(`/artifacts/${encodeURIComponent(id)}`);
});

export const archiveArtifact = command(v.string(), async (id) => {
	const res = await durbarFetch(`/artifacts/${encodeURIComponent(id)}/archive`, {
		method: 'POST'
	});
	if (!res.ok) {
...
export const restoreArtifact = command(v.string(), async (id) => {
	const res = await durbarFetch(`/artifacts/${encodeURIComponent(id)}/restore`, {
		method: 'POST'
	});
	if (!res.ok) {
		const detail = await res.text();

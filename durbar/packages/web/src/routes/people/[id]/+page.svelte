<script lang="ts">
	...
	import { getPerson } from "$lib/remote/people.remote.js";
	let data = $state<unknown>(null); let err = $state<string | null>(null);
	let id = $derived(page.params.id ?? "");
	onMount(async () => { try { data = await getPerson(id); } catch (e) { err = e instanceof Error ? e.message : String(e); } });
</script>
<div class="mx-auto max-w-3xl px-6 py-8">
	<h1 class="text-xl font-semibold text-zinc-100">Person: {page.params.id}</h1>

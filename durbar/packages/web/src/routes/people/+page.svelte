...
<div class="mx-auto max-w-5xl px-6 py-8">
	...
	<ul class="mt-4 space-y-2">{#each people as p (JSON.stringify(p))}<li class="rounded border border-zinc-800 bg-zinc-900 p-3 text-sm text-zinc-300"><pre class="text-xs text-zinc-400">{JSON.stringify(p, null, 2)}</pre></li>{/each}</ul>
</div>

<script lang="ts">
	...
	let workflows: unknown[] = []; let runs: unknown[] = []; let err: string | null = null;
...
<div class="mx-auto max-w-5xl px-6 py-8">
	...
	<ul class="mt-2 space-y-2">{#each workflows as w (JSON.stringify(w))}<li class="rounded border border-zinc-800 bg-zinc-900 p-3 text-sm"><pre class="text-xs text-zinc-400">{JSON.stringify(w, null, 2)}</pre></li>{/each}</ul>
	<h2 class="mt-6 text-sm font-medium text-zinc-200">Recent runs</h2>
	<ul class="mt-2 space-y-2">{#each runs as r (JSON.stringify(r))}<li class="rounded border border-zinc-800 bg-zinc-900 p-3 text-sm"><pre class="text-xs text-zinc-400">{JSON.stringify(r, null, 2)}</pre></li>{/each}</ul>
</div>

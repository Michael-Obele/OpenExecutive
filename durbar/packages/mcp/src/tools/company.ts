...
const memoryKind = v.pipe(
  ...
  v.description("Which episodic memory: `decisions`, `initiatives`, or `advice`."),
);
...
async function ask(input: Record<string, unknown>): Promise<unknown> {
  ...
  if (input.committee_review !== undefined) body.committee_review = input.committee_review;
...
const actions: ActionSpec[] = [
  {
    name: "get_profile",
    description: "Read the saved company profile. A 404 means onboarding has not been committed yet — use `onboard_start`.",
    method: "GET",
    path: "/company-profile",
  },
  {
    name: "patch_profile",
    description: "Update the saved company profile — send only the keys that changed. Fails with 404 when no profile exists yet.",
    method: "PATCH",
    path: "/company-profile",
    fields: {
      profile: jsonObject("Partial company profile (same shape as `get_profile`'s response). Only the keys you send are updated."),
    },
    bodyIs: "profile",
  },
  {
    ...
    fields: {
      description: optionalText("Free-text company description (up to 20k chars). Empty returns the opening prompt instantly."),
    },
  ...
  },
  {
    ...
    description: "Answer the current onboarding question with free text. Returns the next question, or a draft once enough has been collected.",
    method: "POST",
  ...
  {
    name: "onboard_draft",
    description: "Force a profile draft now, however much is still unknown. Use to preview what would be saved before committing.",
    method: "POST",
    path: "/onboard/draft",
  ...
  },
  {
    ...
    fields: {
      ...
      profile: v.optional(v.pipe(v.record(v.string(), v.unknown()), v.description("Company profile patch. All fields optional; merged onto a fresh profile."))),
    },
    bodyFrom: ["session_id", "profile"],
  },
  {
    name: "get_onboarding_session",
    description: "Fetch an in-flight onboarding session: transcript, current question or draft, and whether it was already saved.",
    method: "GET",
  ...
  {
    name: "get_today",
    description: "Today's briefing: the Executive's narrative, proposals awaiting a decision, overnight activity, and at-risk items.",
    method: "GET",
    path: "/today",
  },
  {
    name: "get_activity",
    description: "The activity feed behind the briefing — recent events across agents, integrations and the scheduler.",
    method: "GET",
    path: "/today/activity",
  },
  {
    name: "get_last_turn",
    description: "Debug view of the most recent chat turn: the collected SSE events and turn metadata.",
    method: "GET",
    path: "/sessions",
    fields: {
      limit: v.optional(v.pipe(v.number(), v.description("How many sessions to list (default 10)."))),
    },
    query: ["limit"],
  },
  {
    name: "list_memories",
    description: "List episodic memory entries of one kind — what the company has decided, is running, or was advised.",
    method: "GET",
    path: "/memories/:kind",
  ...
  },
  {
    ...
    description: "Edit one recorded decision — correct its outcome, rationale, summary, domain or tags.",
    method: "PATCH",
  ...
  },
  {
    ...
    description: "Edit one tracked initiative — retitle it, change its status, or rewrite the summary.",
    method: "PATCH",
  ...
  {
    name: "update_advice",
    description: "Edit one recorded piece of advice — amend the question asked, the advice given, or its domain.",
    method: "PATCH",
  ...
  {
    name: "delete_memory",
    description: "⚠️ destructive — permanently delete one episodic memory entry.",
    method: "DELETE",
  ...
  },
  {
    ...
    fields: {
      ...
      session_id: optionalText("Continue an existing chat session; omit to start a new one."),
      committee_review: optionalFlag("Run the committee review pass before answering. Slower, more thorough."),
    },
    custom: ask,
  },
];

export function registerCompanyTools(server: Parameters<typeof defineDomainTool>[0]): void {

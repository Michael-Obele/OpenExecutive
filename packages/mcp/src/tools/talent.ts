 ...
 *
 * The funnel runs engagement → candidate → offer. Offers have real state
 * machines attached (approval for senior comp, nudges, expiry), so the tool
 * exposes the transition actions explicitly rather than a generic patch:
 * `extend_offer` is the only path to `extended`, and terms are editable only
 * while an offer is `draft` or `pending_approval`.
 */
import {
  type ActionSpec,
  defineDomainTool,
...
const actions: ActionSpec[] = [
  // ── engagements (a role being hired for) ─────────────────────────────
  {

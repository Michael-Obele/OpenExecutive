...
// Ported from `packages/ui/src/lib/practice.ts` — logic unchanged.

import type { ClientCockpitCard } from '$lib/api.js';
...
export function clientCountsSummary(c: ClientCockpitCard): string {
	...
	const counts =
		[
			...
			.join(' · ') || 'all quiet';
	const stamp =
		!c.is_active && c.saved_at ? ` · as of ${new Date(c.saved_at).toLocaleDateString()}` : '';
	return counts + stamp;
}

...
// /components/Briefing.svelte): grouping audit rows per alert, the one-line
// summary, and the trailer links. No framework here so scripts/handled.test.mjs
// can exercise it directly.
// Ported from `packages/ui/src/lib/handled.ts` — logic unchanged.
import type { HandledItem } from '$lib/api.js';
...
export function handledHeadline(rows: HandledRow[], reverted: (h: HandledItem) => boolean): string {
	...
	if (counts.suggested > 0)
		parts.push(plural(counts.suggested, 'workflow suggested', 'workflows suggested'));
...
export function handledAlsoLine(row: HandledRow): string {
	const parts: string[] = [];
	if (row.foldedIn > 0)
		parts.push(`${row.foldedIn} duplicate${row.foldedIn === 1 ? '' : 's'} folded in`);
	if (row.also.length > 0)
		parts.push(`also ${row.also.map((a) => handledVerb(a.kind)).join(', ')}`);
	return parts.join(' · ');
}
